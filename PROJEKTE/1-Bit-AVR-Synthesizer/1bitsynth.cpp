...
uint16_t lfofreq = 22;   // lfo period
uint16_t lfodepth = 23;  // lfo wave amplitude
uint8_t envfreq = 55;    // envelope period
uint8_t envwidth = 33;   // envelope duty cycle
volatile uint8_t wavenum = 3; // oscillator waveform
uint8_t lfowavenum = 8; // lfo waveform

// midi state
enum { MIDI_OTHER, MIDI_NOTE_OFF, MIDI_NOTE_ON, MIDI_CC };
...
ISR(USART_RX_vect)
{
  ...
  else
  {
	...
	/////////////////////////////////////////////////////////
	case MIDI_CC:
		if(midibuffer[0] < NUM_KNOBS && midibuffer[1] <=127)
			knobs[midibuffer[0]].midi = midibuffer[1];
	break;
...
void update_pitch()
{
  uint16_t newpitch = (usemidi) ? pitch-(lfoval*4U) : 200U + ((pitch-lfoval)*4U);
...
void update_synth_params()
{
  ...
  if (!usemidi)
    // pitch = freqvals[(knobs[PITCH_KNOB].val >> 3)];
	// if(pitch > 60000) pitch = 60000;
...
int main(void)
{
 ...
 // }
  ...
  {
	  /*
	if(!read_bit(DDRD,5) || !read_bit(DDRD,7)){
		if(!read_bit(DDRD,5)){
			usemidi = !usemidi;
			_delay_ms(500);
		}
		else{
			uint8_t note = 40+(adctimer&15)*3;
