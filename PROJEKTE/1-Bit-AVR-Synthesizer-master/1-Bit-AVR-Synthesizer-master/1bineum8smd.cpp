...
#define PITCH_KNOB        4
#define LFO_FREQ_KNOB     1
#define LFO_DEPTH_KNOB    0
// #define LFO_WF_KNOB       7
#define ENV_WIDTH_KNOB    3
#define ENV_FREQ_KNOB     2
#define EXT_KNOB_1        5
// #define EXT_KNOB_2        8
...
#define EN_BTN	     	 6
#define LFO_WF_BTN       7
#define AUDIO_WF_BTN     5

bool btn_audio, btn_lfo;
...
volatile uint8_t wavenum = 10; // oscillator waveform
...
ISR(TIMER1_COMPA_vect)
{
  ...
  if (wavenum < 7)
  {
    ...
    if (shiftout)
      waveform |= (1 << 15);
	// shiftout = tia_out(wavenum);
  }
  else
  {
	if(wavenum >= 8)
		shiftout = tia_out(wavenum-8);
	else
...
void update_pitch()
{
  uint16_t newpitch = (usemidi || usenotes) ? pitch-(lfoval*4U)-pitchmod
                                : 400U + ((pitch-lfoval)*4U);
  if (newpitch != outpitch)
  {
	if(usemidi){
		
		if(glide){
			// int diff = newpitch - outpitch;
			if(!(((cnt>>1)&31)%(porta))){
			
			if(newpitch > outpitch)
				outpitch = MIN(outpitch+1+porta/8,newpitch);
			else
				outpitch = MAX(outpitch-1+porta/8,newpitch);
			}
		}
		else
			outpitch = newpitch;
...
void update_synth_params()
{
  ...
  if (!usemidi){
	...
	if(usenotes)
		pitch = pgm_read_word(freqvals+(90-((pitch>>6))));
    
    // noteon = rbi(PIND,EN_BTN);
  }
  else{
	// pitch knob changes portamento in midi mode
	porta = knobs[PITCH_KNOB].val / 16;
	}
	
/*   
  // uint8_t newwavenum = knobs[AUDIO_WF_KNOB].val >> 7;
  uint8_t newwavenum = audio_wf;
 ...
 */
  ...
  pitchmod = knobs[EXT_KNOB_1].val >> 3;
  pitchmod -= 64;
...
void demo(){
  ...
  do{
	...
	if((wavenum&1) && lfowavenum)
		lfowavenum--;
		
	for(uchar i=0; i<16; i++){
		// uchar nn = 100 + dn[i>>1] * 32;
		// nn <<= (i&1);
			// usemidi=0;
		
		// knobs[PITCH_KNOB].val = MIN(nn,800);
		// knobs[PITCH_KNOB].val = constrain((i&3)*123,10,1000);
		knobs[LFO_FREQ_KNOB].val = 100;
		// knobs[LFO_DEPTH_KNOB].val = (i>>3)*200;
		knobs[LFO_DEPTH_KNOB].val = i>12 ? 800 : 0;
		knobs[ENV_FREQ_KNOB].val = ((i/2)&3)*200;
		knobs[ENV_WIDTH_KNOB].val = (i&1)?800:100;
		
		mute=false;
		glide = false;
		update_synth_params();
		pitch = pgm_read_word(freqvals+60+(12*(i&3)));
		if( (i/2) & 3 == 0){
			glide = true;
			porta = (i&7)*4;
		}
...
int main(void)
{
  ...
  // buttons
  set_bit(PORT(BTN_PORT), AUDIO_WF_BTN);
  set_bit(PORT(BTN_PORT), LFO_WF_BTN);
  set_bit(PORT(BTN_PORT), EN_BTN);
  btn_audio = btn_lfo = true;
  ...
  usenotes = !rbi(PIND,AUDIO_WF_BTN);
  ...
  for (;;)
  {
	...
	bool b_a = rbi(PIND,AUDIO_WF_BTN);
	bool b_b = rbi(PIND,LFO_WF_BTN);
	
	if( !b_a){
		wavenum++;
		if(wavenum==17) wavenum = 0;

		waveform = waveforms[wavenum];
		OFF(LFO_LED_PIN);
		// OFF(PWR_LED_PIN);
		PORT(LEDS)  |= _BV(ENV_LED_PIN);
		_delay_ms(200);
		PORT(LEDS)  &= ~_BV(ENV_LED_PIN);
		_delay_ms(200);
	}
